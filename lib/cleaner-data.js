...
function parseResolvedIssue(comment = '') {
  return /\[issue-resolved:true\]/i.test(comment);
}

function parseResolutionNote(comment = '') {
  const match = comment.match(/\[resolution-note\]\s*([^\n]+)/i);
  return match ? match[1].trim() : '';
}

function parseExecutionNote(comment = '') {
  return comment
    .replace(/\[grade:\d\/5\]\s*/i, '')
    .replace(/\[initial-grade:\d\/5\]\s*/i, '')
    .replace(/\[issue-resolved:true\]\s*/i, '')
    .replace(/\[resolution-note\]\s*[^\n]+\s*/i, '')
    .trim();
}

function parseAllocationNote(reason = '') {
  const match = String(reason || '').match(/\[allocation-note\]\s*([^\n]+)/i);
  return match ? match[1].trim() : '';
}
...
function mapInstanceTask(instance) {
  ...
  const initialGrade = parseInitialGrade(comment);
  const resolvedIssue = parseResolvedIssue(comment);
  const note = parseExecutionNote(comment);
  const allocationNote = parseAllocationNote(instance.exceptionReason);
  ...
  return {
    ...
    initialGrade,
    resolvedIssue,
    resolutionNote: parseResolutionNote(comment),
    note,
    allocationNote,
    instructionNote: allocationNote || (instance.sourceType === 'ad_hoc' || instance.manuallyCreated ? instance.descriptionSnapshot ?? '' : ''),
    photoCount: instance.execution?.photos?.length ?? 0,
    frequency: instance.taskTemplate?.recurrenceType ?? null,
    taskTemplateId: instance.taskTemplateId ?? null,
    sourceType: instance.sourceType,
    manuallyCreated: Boolean(instance.manuallyCreated),
    isExceptionTask: Boolean(instance.isExceptionTask),
    ...
    boardDayKey: formatBoardDayKey(getTaskInstanceBoardDate(instance), DEFAULT_APP_TIME_ZONE),
    syncVersion: instance.updatedAt?.toISOString?.() ?? null,
...
export async function getCleanerStaffList(staffSlug, selectedBoardDay) {
