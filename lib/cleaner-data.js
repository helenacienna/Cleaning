...
function makeCleanerShiftAssignmentId({ staff, day, facility, zone }) {
...
function buildPrismaAssignment(id, instances) {
  ...
  return {
    ...
    zone: first.plannedZone?.name ?? first.zone.name,
...
async function getPrismaAssignments() {
  ...
  for (const instance of instances) {
    ...
    const zone = instance.plannedZone?.name ?? instance.zone.name;
    const day = instance.shiftRun ? formatBoardDay(instance.shiftRun.runDate) : formatBoardDay(instance.dueAt);

    if (!staff || !facility || !zone) {
      continue;
    }

    const id = makeCleanerShiftAssignmentId({ staff, day, facility, zone });
    if (!grouped.has(id)) {
      grouped.set(id, []);
    }
