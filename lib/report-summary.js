...
export async function loadPeriodReport({ period, facility = '' }) {
  ...
  const scored = tasks.map((task, originalIndex) => {
    ...
    return {
      ...
      serviceLevel: serviceLevelKey(task),
      photoCount,
...
export function emptyTotals() {
  return {
    ...
    serviceLevels: { check: 0, clean: 0, detailed_clean: 0 },
  };
}
...
export function buildTotals(scored) {
  ...
  scored.forEach((entry) => {
    ...
    const levelKey = entry.serviceLevel === 'check' || entry.serviceLevel === 'detailed_clean' ? entry.serviceLevel : 'clean';
    totals.serviceLevels[levelKey] = (totals.serviceLevels[levelKey] ?? 0) + 1;
...
function appendPeriodParam(route, key, value) {
  const separator = route.includes('?') ? '&' : '?';
  return `${route}${separator}${key}=${value}`;
}

export function previousPeriodHref(period, route) {
  if (period.type === 'weekly') {
    return appendPeriodParam(route, 'week', toDayKey(addDays(period.start, -7)));
  }
  const previous = new Date(Date.UTC(period.start.getUTCFullYear(), period.start.getUTCMonth() - 1, 1));
  return appendPeriodParam(route, 'month', toDayKey(previous).slice(0, 7));
}
...
export function nextPeriodHref(period, route) {
  ...
  return appendPeriodParam(route, 'month', toDayKey(next).slice(0, 7));
}
