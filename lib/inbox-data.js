...
function mapParticipantRecord(participant) {
  ...
  return {
    ...
    role: staffRole ? formatStaffRole(staffRole) : participant.role.toLowerCase(),
...
function mapThreadRecord(thread, participantStaffCode = '') {
  const orderedMessages = [...(thread.messages ?? [])].sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
  ...
  return {
    ...
    scope: thread.scope,
    status: thread.status,
    unreadCount: participantStaffCode
      ? (thread.participants.find((participant) => participant.staff?.staffCode === participantStaffCode)?.unreadCount ?? 0)
      : thread.participants.reduce((sum, participant) => sum + participant.unreadCount, 0),
    participantCount: thread.participants.length,
...
async function listPrismaThreads(prisma, { audience = 'manager', limit = 12, participantStaffCode = '' } = {}) {
  const where = {
    ...(audience ? { audience } : {}),
    ...(participantStaffCode ? { participants: { some: { staff: { staffCode: participantStaffCode }, archivedAt: null } } } : {}),
  };
  ...
  return threads.map((thread) => mapThreadRecord({ ...thread, messages: [] }, participantStaffCode));
}

async function listInboxStaffOptions(prisma, audience = 'manager') {
  const roles = audience === 'staff'
  ...
  return staff.map((member) => ({
    ...
    label: `${member.fullName} · ${formatStaffRole(member.role)}`,
  }));
}

async function getPrismaThreadDetail(prisma, threadId, options = {}) {
  ...
  if (options.participantStaffCode && !thread.participants.some((participant) => participant.staff?.staffCode === options.participantStaffCode)) {
    return null;
  }
  return mapThreadRecord(thread, options.participantStaffCode || '');
}
...
export async function getInboxWorkspaceData(selectedThreadId, options = {}) {
  ...
  try {
    ...
    if (!threads.length) {
      return {
        ...getFallbackInboxWorkspace(selectedThreadId),
        source: 'prisma',
        composerDefaults: { senderOptions: [], participantOptions },
      };
    }

    const selectedId = selectedThreadId ?? threads[0].id;
    const selectedThread = await getPrismaThreadDetail(prisma, selectedId, options); 

    return {
      threads,
      selectedThread: selectedThread ?? (await getPrismaThreadDetail(prisma, threads[0].id, options)),
...
export async function createInboxReply({ threadId, senderStaffCode, body, attachments = [] }) {
  ...
  try {
    ...
    if (sender && !thread.participants.some((participant) => participant.participantKey === senderKey && !participant.archivedAt)) {
      throw new Error('Thread not found');
    }
...
export async function createInboxThread({ title, subtitle, audience = 'manager', participantStaffCodes = [], senderStaffCode }) {
  ...
  try {
    ...
    const sender = senderStaffCode
      ? await prisma.staff.findUnique({ where: { staffCode: senderStaffCode } })
      : null;
    const participantMap = new Map(staffMembers.map((staff) => [staff.staffCode, staff]));
    if (sender) participantMap.set(sender.staffCode, sender);
    ...
    const participants = [
      ...Array.from(participantMap.values()).map((staff) => ({ staff, role: sender?.staffCode === staff.staffCode ? 'owner' : 'member' })),
...
export async function findOrCreateStaffDirectThread({ staffCode, senderStaffCode = '' }) {
  const targetCode = String(staffCode || '').trim().toUpperCase();
  const senderCode = String(senderStaffCode || '').trim().toUpperCase();
  if (!targetCode) {
  ...
  const [targetStaff, senderStaff] = await Promise.all([
    prisma.staff.findFirst({ where: { staffCode: targetCode, active: true } }),
    senderCode ? prisma.staff.findFirst({ where: { staffCode: senderCode, active: true } }) : prisma.staff.findFirst({ where: { active: true, role: 'manager' }, orderBy: { fullName: 'asc' } }),
  ]);
  ...
  const participantCodes = [senderStaff?.staffCode, targetStaff.staffCode].filter(Boolean).sort();
  const threadKey = `staff-direct:${participantCodes.join(':')}`;

  const existingThread = await prisma.inboxThread.findUnique({ where: { threadKey } }).catch(() => null);
  if (existingThread) {
    return {
      ok: true,
      source: 'prisma',
      thread: await getPrismaThreadDetail(prisma, existingThread.id),
    };
  }
  ...
  const thread = await prisma.inboxThread.create({
    data: {
      ...
      type: 'direct',
      audience: 'staff',
      title: targetStaff.fullName,
      subtitle: senderStaff ? `Direct message with ${targetStaff.fullName}` : `Direct staff message`,
      scope: 'manual',
      status: 'open',
    },
  });
  ...
  const participants = [
    ...(senderStaff ? [{ staff: senderStaff, role: 'owner' }] : []),
    { staff: targetStaff, role: senderStaff?.staffCode === targetStaff.staffCode ? 'owner' : 'member' },
    { key: 'system:ops', name: 'Operations system', role: 'system' },
  ];
