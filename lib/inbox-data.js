...
import { cleanerAssignments, supervisorCards } from '../data/demo-data';
...
function getDemoMessages() {
  ...
  return [
      ...
      messages: [
        {
          ...
          senderRole: 'system',
          kind: 'alert',
          body: '2 open exceptions still need manager action across Cienna North and Cienna Central.',
        ...
        },
        {
          ...
          senderRole: 'system',
          kind: 'alert',
          body: 'Late shift completion dipped below target at Cienna Central: 74% complete.',
    ...
    },
    {
      ...
      subtitle: 'Cienna North · Toilet block',
      scope: 'issue',
      status: 'open',
      participants: DEMO_PARTICIPANTS,
      unreadCount: 1,
    ...
    },
    {
      ...
      subtitle: 'Cienna Central · Audit score recovery',
      scope: 'audit',
      status: 'open',
      participants: DEMO_PARTICIPANTS,
      unreadCount: 0,
...
function getFallbackInboxWorkspace(selectedThreadId) {
  ...
  return {
    ...
    source: 'demo',
    composerDefaults: {
      senderOptions: DEMO_PARTICIPANTS.filter((participant) => participant.role !== 'system').map((participant) => ({
...
async function buildDefaultOperationalParticipants(prisma, audience = 'manager') {
  const staff = await prisma.staff.findMany({
    where: {
      active: true,
      role: { in: audience === 'supervisor' ? ['supervisor'] : ['manager', 'supervisor'] },
    },
...
export async function createInboxReply({ threadId, senderStaffCode, body }) {
  if (!body?.trim()) {
    throw new Error('Message body is required');
  }

  if (!shouldUsePrisma()) {
    const thread = getDemoThreads().find((item) => item.id === threadId);
    return {
      ok: true,
      source: 'demo',
      threadId,
      message: {
        id: crypto.randomUUID(),
        senderKey: senderStaffCode ? `staff:${senderStaffCode}` : 'staff:MGR001',
        senderName: senderStaffCode === 'SUP001' ? 'Daniel Price' : 'Olivia Hart',
        senderRole: senderStaffCode === 'SUP001' ? 'supervisor' : 'manager',
        kind: 'note',
        body: body.trim(),
        createdAt: new Date().toISOString(),
        formattedTime: formatThreadTime(new Date()),
      },
      thread: thread ?? null,
    };
  }
  ...
  const prisma = await getPrisma();
  if (!prisma) {
    const senderName = senderStaffCode === 'SUP001' ? 'Daniel Price' : 'Olivia Hart';
    const senderRole = senderStaffCode === 'SUP001' ? 'supervisor' : 'manager';
    return {
      ok: true,
      source: 'demo',
      threadId,
  ...
  try {
    ...
    const message = await prisma.inboxMessage.create({
      data: {
        ...
        kind: 'note',
        body: body.trim(),
        metadata: sender ? { senderRole: sender.role } : { senderRole: 'system' },
      },
    });
...
export async function createInboxThread({ title, subtitle, audience = 'manager', participantStaffCodes = [], senderStaffCode }) {
  ...
  if (!shouldUsePrisma()) {
    return {
      ok: true,
      source: 'demo',
      thread: mapDemoThread({
        id: crypto.randomUUID(),
        threadKey: null,
        type: 'group',
        audience,
        title: title.trim(),
        subtitle: subtitle?.trim() || 'New internal thread',
        scope: 'manual',
        status: 'open',
        participants: DEMO_PARTICIPANTS,
        unreadCount: 0,
        ...
        messages: subtitle?.trim() ? [{
          id: crypto.randomUUID(),
          senderKey: senderStaffCode || 'staff:MGR001',
          senderName: 'Olivia Hart',
          senderRole: 'manager',
          kind: 'note',
          body: subtitle.trim(),
