      ...
      messages: [
        {
          ...
          attachments: [
            { label: 'Open organiser board', url: '/admin/daily-hierarchy', type: 'internal' },
            { label: 'Open cleaner checklist', url: '/scan/assignment-1', type: 'internal' },
          ],
...
export async function setInboxThreadStatus({ threadId, status }) {
  ...
  if (!shouldUsePrisma()) {
    return { ok: true, source: 'demo', status };
  }
