import { scheduleBuilder } from '../data/demo-data.js';
import { getPrisma } from './prisma.js';
import { getCleanerAssignments } from './cleaner-data.js';
import { getInboxDemoSnapshot, listInboxThreads } from './inbox-data.js';
import { recordNotification } from './notification-center.js';
import { runRuntimeMaintenance } from './runtime-maintenance.js';
import { getOutcomeCounts } from './task-outcomes.js';
...
export async function getManagerOverviewData() {
  ...
  try {
    const maintenance = await runRuntimeMaintenance(prisma, { readOnly: true });
    ...
    const facilitySummary = Array.from(
      assignments.reduce((map, shift) => {
        ...
        existing.tasks.push(...shift.tasks);
        existing.lowScores += shift.tasks.filter((task) => !task.resolvedIssue && (task.score ?? 0) <= 2 && task.score !== null).length;
        existing.openIssues += shift.tasks.filter((task) => task.status !== 'completed' && !task.resolvedIssue && ((task.score ?? 0) <= 2 && task.score !== null)).length;
      ...
      outcomeCounts: getOutcomeCounts(facility.tasks),
    ...
    const operationsInbox = await listInboxThreads({ audience: 'manager', limit: 8 });
    const unreadInboxCount = operationsInbox.reduce((sum, thread) => sum + thread.unreadCount, 0);
    const supervisorInbox = await listInboxThreads({ audience: 'supervisor', limit: 8 });
    const cleanerInbox = await listInboxThreads({ audience: 'cleaner', limit: 8 });
    ...
    return {
      ...
      operationsInbox,
      unreadInboxCount,
      supervisorInboxCount: supervisorInbox.reduce((sum, thread) => sum + thread.unreadCount, 0),
      cleanerInboxCount: cleanerInbox.reduce((sum, thread) => sum + thread.unreadCount, 0),
      reportingCards,
