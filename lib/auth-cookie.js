...
export function signAuthSession({ role, username = '', name = '', staffSlug = '', issuedAt = Date.now() }) {
  ...
  const payload = base64UrlEncode(JSON.stringify({ role, username, name, staffSlug, issuedAt }));
...
export function getUserForCredentials({ username = '', password = '' } = {}) {
  ...
  if (process.env.CLEANING_ADMIN_PASSWORD && password === process.env.CLEANING_ADMIN_PASSWORD) {
    return { username: normalizedUsername || 'shared-admin', role: 'admin', name: 'Admin', staffSlug: '' };
  }

  if (process.env.CLEANING_STAFF_PASSWORD && password === process.env.CLEANING_STAFF_PASSWORD) {
    return { username: normalizedUsername || 'shared-staff', role: 'staff', name: 'Staff', staffSlug: '' };
  }
