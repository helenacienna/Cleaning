...
export async function ensureUpcomingTaskInstances(tx, options = {}) {
  ...
  const configuredHorizonMonths = Number.parseInt(process.env.TASK_INSTANCE_HORIZON_MONTHS ?? '', 10);
  ...
  const horizonDays = Number.isInteger(options.horizonDays)
    ? options.horizonDays
    : (Number.isInteger(configuredHorizonDays) ? configuredHorizonDays : null);
  const horizonMonths = Number.isInteger(configuredHorizonMonths) ? configuredHorizonMonths : 13;
  const horizon = Number.isInteger(horizonDays)
    ? addDays(now, horizonDays)
    : addMonths(now, horizonMonths);
  const horizonSpanDays = Math.max(1, Math.ceil((horizon.getTime() - now.getTime()) / (24 * 60 * 60 * 1000)));
  const generationGuardLimit = Math.max(60, horizonSpanDays + 31);
  ...
  for (const template of templates) {
    ...
    while (cursor <= horizon && guard < generationGuardLimit) {
      const existing = await tx.taskInstance.findFirst({
        where: {
