...
async function getDefaultDailyAssignment(tx, taskTemplate, dueAt) {
  if (taskTemplate?.recurrenceType !== 'daily') {
    return {};
  }

  const facility = await tx.facility.findUnique({
    where: { id: taskTemplate.facilityId },
    select: { name: true },
  });

  if (facility?.name !== 'Cienna') {
    return {};
  }

  const staff = await tx.staff.findFirst({
    where: { fullName: 'Tony', active: true },
    select: { id: true },
  });
  ...
  return {
    ...
    plannedRunDate: new Date(`${new Date(new Date(dueAt).getTime() + 10 * 60 * 60 * 1000).toISOString().slice(0, 10)}T00:00:00+10:00`),
  };
}
...
export async function ensureFutureTaskInstance(tx, taskTemplate, dueAt) {
  ...
  const defaultAssignment = await getDefaultDailyAssignment(tx, taskTemplate, dueAt);
