...
import { getIssueState, parseInitialGradeFromComment, parseResolvedIssueFromComment } from './task-outcomes.js';
...
export function mapTaskTemplateToLibraryCard(taskTemplate) {
  return {
    ...
    taskGroupId: taskTemplate.taskGroup?.id ?? null,
    zone: taskTemplate.zone?.name ?? '',
    zoneId: taskTemplate.zone?.id ?? null,
    facility: taskTemplate.facility?.name ?? '',
    facilityId: taskTemplate.facility?.id ?? null,
    serviceType: taskTemplate.serviceType ?? 'routine',
    ...
    designatedDay: taskTemplate.recurrenceType === 'weekly'
      ...
      : '—',
    frequencyType: taskTemplate.priority === 'critical' ? 'Critical' : taskTemplate.priority === 'optional' ? 'Optional' : 'Standard',
    ...
    passCriteria: taskTemplate.passCriteria ?? '',
    safetyNotes: taskTemplate.safetyNotes ?? '',
    autoGenerateInstances: taskTemplate.autoGenerateInstances,
    requiresPlanning: taskTemplate.requiresPlanning,
    canBeSplit: taskTemplate.canBeSplit,
    canBeMovedBetweenStaff: taskTemplate.canBeMovedBetweenStaff,
    requiresManagerApprovalToSkip: taskTemplate.requiresManagerApprovalToSkip,
    missedTaskPolicy: taskTemplate.missedTaskPolicy,
    rescheduleWindowDays: taskTemplate.rescheduleWindowDays ?? '',
...
function buildPrismaOrganiserBoard({ staffMembers, shiftRuns, taskInstances, automationSummary, validationSummary, templateRecommendations = new Map(), timeZone = DEFAULT_APP_TIME_ZONE }) {
  ...
  const cards = safeTaskInstances.map((instance, index) => {
    ...
    const completionComment = instance.execution?.completionComment ?? '';
    const executionGrade = parseExecutionGrade(completionComment);
    const initialGrade = parseInitialGradeFromComment(completionComment);
    const resolvedIssue = parseResolvedIssueFromComment(completionComment);
    const reworkRequired = latestAudit?.reworkRequired || instance.status === 'carried_forward';
    const issueState = getIssueState({
      issueRaised: instance.execution?.issueRaised,
      initialGrade,
      score: executionGrade,
      reworkRequired,
      managerAction: latestAudit?.managerAction,
      resolvedIssue,
    });
    ...
    return {
      ...
      taskTemplateUuid: instance.taskTemplateId,
      ...
      serviceLevel: instance.serviceLevel ?? instance.taskTemplate?.serviceLevel ?? 'clean',
      ...
      manuallyCreated: Boolean(instance.manuallyCreated),
      addedToday: instance.sourceType === 'ad_hoc' || Boolean(instance.manuallyCreated),
      ...
      score: executionGrade ?? latestAudit?.auditScore ?? null,
      auditScore: latestAudit?.auditScore ?? executionGrade ?? null,
      initialGrade,
      ...issueState,
...
export async function getOrganiserBoardData(options = {}) {
  ...
  try {
    ...
    board.taskTemplates = taskTemplates.map(mapTaskTemplateToLibraryCard);
