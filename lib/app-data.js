...
function buildPrismaOrganiserBoard({ staffMembers, shiftRuns, taskInstances, automationSummary, validationSummary }) {
  const days = [...new Set(taskInstances.map((instance) => formatBoardDayKey(instance.shiftRun?.runDate ?? instance.dueAt)))];
...
export async function getOrganiserBoardData() {
  ...
  try {
    ...
    const [managerInbox, supervisorInbox, cleanerInbox] = await Promise.all([
      listInboxThreads({ audience: 'manager', limit: 12 }),
      listInboxThreads({ audience: 'supervisor', limit: 12 }),
      listInboxThreads({ audience: 'cleaner', limit: 12 }),
    ]);

    const validationSummary = validateTaskInstanceSequence(taskInstances);
