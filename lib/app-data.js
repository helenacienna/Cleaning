...
import { parseInitialGradeFromComment, parseResolvedIssueFromComment } from './task-outcomes.js';
...
function buildPrismaOrganiserBoard({ staffMembers, shiftRuns, taskInstances, automationSummary, validationSummary, templateRecommendations = new Map(), timeZone = DEFAULT_APP_TIME_ZONE }) {
  ...
  const cards = safeTaskInstances.map((instance, index) => {
    ...
    const hadIssue = Boolean(instance.execution?.issueRaised)
      || Boolean(initialGrade)
      || (Number.isFinite(executionGrade) && executionGrade <= 2)
      || Boolean(reworkRequired);
    const hasOpenIssue = hadIssue && !resolvedIssue && latestAudit?.managerAction !== 'close';
