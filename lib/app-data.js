...
function buildPrismaOrganiserBoard({ staffMembers, shiftRuns, taskInstances, automationSummary, validationSummary, templateRecommendations = new Map(), timeZone = DEFAULT_APP_TIME_ZONE }) {
  ...
  const cards = safeTaskInstances.map((instance, index) => {
    ...
    const executionGrade = parseExecutionGrade(instance.execution?.completionComment ?? '');
    ...
    return {
      ...
      initialGrade: parseInitialGradeFromComment(instance.execution?.completionComment ?? ''),
      resolvedIssue: parseResolvedIssueFromComment(instance.execution?.completionComment ?? ''),
