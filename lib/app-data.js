...
export function mapTaskTemplateToLibraryCard(taskTemplate) {
  const priorityValue = taskTemplate.priority === 'critical'
    ? 'critical'
    : taskTemplate.priority === 'optional'
      ? 'optional'
      : 'standard';

  const frequencyType = priorityValue === 'critical'
    ? 'Critical'
    : priorityValue === 'optional'
      ? 'Optional'
      : 'Standard';

  const evidenceRequirement = taskTemplate.evidenceRequirement ?? 'none';
  const commentRequirement = taskTemplate.commentRequirement ?? 'none';
  ...
  return {
    ...
    frequencyType,
    priorityValue,
    required: evidenceRequirement === 'required_photo'
      ? 'Forced photo'
  ...
  try {
    ...
    return {
      facilities: facilities.map((facility) => ({
        ...
        zoneCount: facility._count?.zones ?? 0,
        templateCount: facility._count?.taskTemplates ?? 0,
        taskInstanceCount: facility._count?.taskInstances ?? 0,
      })),
      source: 'prisma',
    };
  } catch {
