...
function ServiceLevelChoiceGroup({ value, disabled = false, onChange }) {
  return (
    <div style={serviceLevelGroupStyle}>
      <strong>Cleaning level</strong>
      <div style={serviceLevelChoiceListStyle} aria-label="Cleaning level">
        {SERVICE_LEVEL_OPTIONS.map((option) => (
          <button
            key={option.value}
            type="button"
            className={value === option.value ? 'button primary' : 'button secondary'}
            style={serviceLevelChoiceStyle}
...
export default function FacilityAddTaskButton({ facility, day }) {
  ...
  async function openAddTaskPopup() {
    ...
    try {
      const [taskResponse, staffResponse] = await Promise.all([
        fetch(`/api/task-library?facility=${encodeURIComponent(facility)}&day=${encodeURIComponent(day)}`, { cache: 'no-store' }),
  ...
  async function addTaskForToday(payload, staffMember = null) {
    if (!day || addTaskState.saving) return;
    ...
    try {
      ...
      setAddTaskState((current) => ({
        ...current,
        saving: false,
        success: result.alreadyScheduled ? 'That task is already on today’s list.' : `Added to ${staffMember?.fullName || 'today’s active list'}.`,
      }));
  ...
  return (
    <>
      <button className="button primary facility-board-add-task-button" type="button" onClick={openAddTaskPopup} disabled={!day}>
        Add task
      </button>
      {addTaskState.open ? (
                ...
                <>
                  ...
                  <div style={zonePickerStackStyle}>
                    ...
                    <div style={taskCardListStyle} aria-label={selectedZone ? `${selectedZone} tasks` : 'Zone tasks'}>
                      {selectedZone ? <div className="muted" style={{ marginBottom: 4 }}>{selectedZone} · {selectedZoneTasks.length} matching tasks</div> : null}
...
const addTaskBackdropStyle = { zIndex: 1200, background: 'rgba(2,6,23,0.72)' };
const addTaskCardStyle = { width: '100vw', height: '100dvh', maxHeight: '100dvh', overflow: 'hidden', borderRadius: 0, boxShadow: 'none', display: 'grid', gridTemplateRows: 'auto minmax(0, 1fr) auto', gap: 12 };
const addTaskHeaderStyle = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, paddingBottom: 10, borderBottom: '1px solid rgba(148,163,184,0.28)' };
const addTaskPanelStyle = { display: 'grid', gridTemplateRows: 'auto minmax(0, 1fr)', alignContent: 'stretch', gap: 10, minHeight: 0, height: '100%', overflow: 'hidden' };
const zonePickerStackStyle = { display: 'grid', gridTemplateRows: 'auto minmax(0, 1fr)', gap: 10, alignItems: 'stretch', minHeight: 0, height: '100%' };
const zoneListStyle = { display: 'flex', gap: 8, overflowX: 'auto', overflowY: 'hidden', paddingBottom: 8, WebkitOverflowScrolling: 'touch' };
const zoneChoiceStyle = { display: 'grid', justifyItems: 'start', textAlign: 'left', gap: 2, whiteSpace: 'normal', minWidth: 150, maxWidth: 190, flex: '0 0 auto' };
const taskCardListStyle = { display: 'grid', gap: 8, minHeight: 0, overflow: 'auto', paddingRight: 4, paddingBottom: 'calc(10mm + env(safe-area-inset-bottom, 0px))', scrollPaddingBottom: 'calc(10mm + env(safe-area-inset-bottom, 0px))' };
...
const taskCardChoiceStyle = { display: 'grid', alignItems: 'center', justifyItems: 'stretch', textAlign: 'left', gap: 3, minHeight: 52, whiteSpace: 'normal' };
...
const serviceLevelGroupStyle = {
  display: 'grid',
  gap: 8,
};

const serviceLevelChoiceListStyle = {
  display: 'grid',
  gap: 8,
};

const serviceLevelChoiceStyle = {
  display: 'grid',
  justifyItems: 'start',
  textAlign: 'left',
  gap: 2,
